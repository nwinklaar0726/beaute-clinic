...
import { useServices, useAvailableSlots } from '@/features/services/hooks/useServices';
...
export function BookingWizard() {
  ...
  const { data: availableSlots = [], isLoading: isLoadingSlots } = useAvailableSlots(
    formData.date?.toISOString().split('T')[0] || null
  );
  ...
  const handleSubmit = async () => {
    ...
    createAppointment.mutate(
      ...
      {
        onSuccess: () => {
          toast.success('¡Cita agendada exitosamente! Te enviamos un correo de confirmación.');
          setStep(1);
          setFormData({
            name: '',
            email: '',
  ...
  const timeSlots = availableSlots.map(time => ({ time, available: true }));
            ...
            <div className="space-y-6 animate-fade-in">
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {isLoadingServices ? (
                    [1, 2, 3, 4].map((i) => (
                      <div key={i} className="p-4 rounded-xl border-2 border-beute-taupe-light/30 bg-white/50 animate-pulse h-24" />
                    ))
                  ) : (
